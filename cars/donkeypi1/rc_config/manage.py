...
def drive(cfg, use_joystick=False, camera_type='single', meta=[]):
    ...
    if (
        cfg.CONTROLLER_TYPE == "wl_cars_rc_controller"
    ):  # an RC controllers read by GPIO pins. They typically don't have buttons
        print("Using WL cars controller")
        from wl_cars_rc_controller import WlCarsRCReceiver

        ctr = WlCarsRCReceiver(cfg)
    elif (
        cfg.CONTROLLER_TYPE == "pigpio_rc"
    ):  # an RC controllers read by GPIO pins. They typically don't have buttons
        from donkeycar.parts.controller import RCReceiver

        print("Using pigpio_rc")
        ctr = RCReceiver(cfg)
    else:
