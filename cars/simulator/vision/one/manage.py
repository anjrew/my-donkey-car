...
def drive(cfg, use_joystick=False, camera_type="single", meta=[]):
    ...
    def inc_pid_i():
        pid.Ki += cfg.PID_I_DELTA
        logging.info("pid: i+ %f" % pid.Ki)
    ...
    if cfg.TOGGLE_RECORDING_BTN:
        ...
        if cfg.TOGGLE_RECORDING_BTN.startswith("web/w"):
            V.add(
                Lambda(lambda: recording_control.toggle_recording()),
    ...
    if (
        hasattr(cfg, 'INC_PID_I_BTN')
        and cfg.INC_PID_I_BTN
        and hasattr(cfg, 'PID_I_DELTA')
        and cfg.PID_I_DELTA
    ):
        print(f"Increment PID I button is {cfg.INC_PID_P_BTN}")
        if cfg.INC_PID_I_BTN.startswith("web/w"):
            V.add(Lambda(lambda: inc_pid_p()), run_condition=cfg.INC_PID_I_BTN)
...
def add_cv_controller(
    ...
    V.add(
        my_class(pid, cfg), inputs=inputs, outputs=outputs, run_condition=run_condition
    )
