...
def load_config(config_file_path: str) -> Tuple:
    with open(config_file_path, 'r') as file:
        config = {}
        exec(file.read(), config)
        return config.get('COLOR_THRESHOLD_LOW'), config.get('COLOR_THRESHOLD_HIGH')
...
def show_pixel_values(image, x, y):
    ...
    if image is not None:
        ...
        if hsv_low is not None and hsv_high is not None:
            ...
            text.append(
                f"Mask HSV High = ({hsv_high[0]}, {hsv_high[1]}, {hsv_high[2]})"
            )
        ...
        for y in range(top, bottom):
            for x in range(left, right):
                ...
                if pixel[0] > 0:  # reject black noise pixels
                    min_pixel = (
                        pixel if min_pixel is None else np.minimum(pixel, min_pixel)
                    )
                    max_pixel = (
                        pixel if max_pixel is None else np.maximum(pixel, max_pixel)
                    )
...
def main(camera_index=0, width=640, height=480, file_image=None, config_path=None):
    ...
    if config_path is not None:
        ...
        if hsv_low is not None and hsv_high is not None:
            set_hsv_trackbars(hsv_low, hsv_high)
    ...
    while 1:
        ...
        if drag_rect is not None:
            top_left = (
                min(drag_rect[0], drag_rect[2]),
                min(drag_rect[1], drag_rect[3]),
            )
            bottom_right = (
                max(drag_rect[0], drag_rect[2]),
                max(drag_rect[1], drag_rect[3]),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-wd", "--width", type=int, default=640, help="width of image to capture"
    )
    parser.add_argument(
        "-ht", "--height", type=int, default=480, help="height of image to capture"
    )
    parser.add_argument(
        "-f", "--file", type=str, help="path to image file to user rather that a camera"
    )
    parser.add_argument(
        "-cfg", "--config", type=str, help="path to the configuration file"
    )
